...
TEST_CASE("job task idles without work and sends first non-clean work at once",
          "[mining][characterization][job-task]")
{
    ...
    // The idle wait is a notification wait, not a task delay. The first
    // dispatch always programs the ASIC mask, even an empty one.
    TEST_ASSERT_EQUAL_UINT32(0, result.delay_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.job_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.coinbase_decode_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.version_mask_count);
    TEST_ASSERT_EQUAL_HEX32(0, result.version_masks[0]);
...
TEST_CASE("job allocation failure retries the same header",
          "[mining][job-building][job-task]")
{
    ...
    // Failed build, retried build, then the next header prepared while the
    // first one hashes. No extranonce value is skipped.
    TEST_ASSERT_EQUAL_UINT32(3, result.allocation_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.job_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.coinbase_decode_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.version_mask_count);
...
TEST_CASE("large coinbase job hashes without temporary buffers and keeps extranonce order",
          "[mining][job-building][job-task]")
{
    ...
    TEST_ASSERT_EQUAL_UINT32(2, result.job_count);
    // Two sent jobs plus the next prepared one; hashing streams the coinbase.
    TEST_ASSERT_EQUAL_UINT32(3, result.allocation_count);
    TEST_ASSERT_EQUAL_UINT32(1, result.coinbase_decode_count);
    TEST_ASSERT_EQUAL_STRING("00", result.jobs[0]->extranonce2);
