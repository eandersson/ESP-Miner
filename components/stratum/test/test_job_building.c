...
TEST_CASE("coinbase hash streams segments byte exact without allocating",
          "[mining][job-building]")
{
...
TEST_CASE("coinbase hash is unaffected by allocation failures",
          "[mining][job-building]")
{
    static uint8_t bytes[1025];
    uint8_t hash[32];
    ...
    // An injected failure has nothing to fail: the streaming hash never
    // allocates, so work generation cannot degrade under heap pressure.
