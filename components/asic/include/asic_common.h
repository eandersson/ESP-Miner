...
// Rebuild the block version a chip hashed. BM13xx chips report the values of
// the rolled mask bits; BM1397 reports which host-rolled midstate matched.
uint32_t asic_rolled_version_from_bits(uint32_t base_version, uint32_t version_mask,
                                       uint32_t version_bits);
uint32_t asic_rolled_version_from_midstate(uint32_t base_version, uint32_t version_mask,
