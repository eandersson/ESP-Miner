...
static bool BM1370_write_clock_delay_ctrl(uint8_t chip_addr, uint8_t header)
{
    return _send_BM1370(header,
                        (uint8_t[]){chip_addr, CORE_REGISTER_CONTROL, 0x80, 0x00, 0x80, BM1370_CLOCK_DELAY_CTRL_VALUE},
                        6,
                        BM1370_SERIALTX_DEBUG);
}

static bool BM1370_write_io_driver_strength(uint8_t chip_addr, uint8_t header, uint8_t value2)
{
    return _send_BM1370(header,
...
uint8_t BM1370_init(GlobalState * GLOBAL_STATE)
{
    ...
    BM1370_INIT_REQUIRE_SENT(
        _send_BM1370((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                     (uint8_t[]){0x00, MISC_CONTROL, 0xF0, 0x00, 0xC1, 0x00}, 6,
                     BM1370_SERIALTX_DEBUG),
    ...
    BM1370_INIT_REQUIRE_SENT(
        _send_BM1370((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                     (uint8_t[]){0x00, CORE_REGISTER_CONTROL, 0x80, 0x00, 0x8B, 0x00}, 6,
                     BM1370_SERIALTX_DEBUG),
    ...
    BM1370_INIT_REQUIRE_SENT(
        _send_BM1370((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                     (uint8_t[]){0x00, ANALOG_MUX_CONTROL, 0x00, 0x00, 0x00, 0x03}, 6,
                     BM1370_SERIALTX_DEBUG),
        "analog mux");
    ...
    BM1370_INIT_REQUIRE_SENT(
        _send_BM1370((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                     (uint8_t[]){0x00, PLL3_PARAMETER, 0x5A, 0xA5, 0x5A, 0xA5}, 6,
                     BM1370_SERIALTX_DEBUG),
        "PLL3 parameter");
    ...
    for (uint8_t i = 0; i < chip_counter; i++) {
        ...
        BM1370_INIT_REQUIRE_SENT(
            _send_BM1370((TYPE_CMD | GROUP_SINGLE | CMD_WRITE),
                         (uint8_t[]){chip_addr, 0xA8, 0x00, 0x07, 0x01, 0xF0}, 6,
                         BM1370_SERIALTX_DEBUG),
            "per-chip register A8");
        //TX: 55 AA 41 09 00 [18 F0 00 C1 00] 0C    // Misc Control
        BM1370_INIT_REQUIRE_SENT(
            _send_BM1370((TYPE_CMD | GROUP_SINGLE | CMD_WRITE),
                         (uint8_t[]){chip_addr, MISC_CONTROL, 0xF0, 0x00, 0xC1, 0x00}, 6,
                         BM1370_SERIALTX_DEBUG),
            "per-chip misc control");
        //TX: 55 AA 41 09 00 [3C 80 00 8B 00] 1A    // Core Register Control
        BM1370_INIT_REQUIRE_SENT(
            _send_BM1370((TYPE_CMD | GROUP_SINGLE | CMD_WRITE),
                         (uint8_t[]){chip_addr, CORE_REGISTER_CONTROL, 0x80, 0x00, 0x8B, 0x00}, 6,
                         BM1370_SERIALTX_DEBUG),
        ...
        BM1370_INIT_REQUIRE_SENT(
            ...
            "per-chip clock delay control");
        asic_init_core_register_delay();
        //TX: 55 AA 41 09 00 [3C 80 00 82 AA] 05    // Core Register Control
        BM1370_INIT_REQUIRE_SENT(
            _send_BM1370((TYPE_CMD | GROUP_SINGLE | CMD_WRITE),
                         (uint8_t[]){chip_addr, CORE_REGISTER_CONTROL, 0x80, 0x00, 0x82, 0xAA}, 6,
                         BM1370_SERIALTX_DEBUG),
    ...
    BM1370_INIT_REQUIRE_SENT(
        _send_BM1370((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                     (uint8_t[]){0x00, ANALOG_MUX_CONTROL, 0x00, 0x00, 0x00, 0x02}, 6,
                     BM1370_SERIALTX_DEBUG),
        "analog mux temp diode");
    ...
    BM1370_INIT_REQUIRE_SENT(
        _send_BM1370((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                     (uint8_t[]){0x00, CORE_REGISTER_CONTROL, 0x80, 0x00, 0x8D, 0xEE}, 6,
                     BM1370_SERIALTX_DEBUG),
        "final core control");
    asic_init_core_register_delay();
