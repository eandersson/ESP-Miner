          ...
          "[asic][version-rolling][characterization]")
{
    ...
    for (size_t d = 0; d < BM13XX_HARNESS_DRIVER_COUNT; d++) {
        ...
        for (size_t c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
            ...
            TEST_ASSERT_EQUAL_INT(ESP_OK, bm13xx_harness_drivers[d].set_version_mask(cases[c].mask));
          ...
          "[asic][version-rolling][characterization]")
{
    static const struct {
        ...
        /* BIP310: the returned bits replace the rolled field, so a returned
         * zero clears a base bit instead of being OR-ed onto it. */
        {0x21002004, {0x00, 0x01}, 0x20002004},
        {0x21002004, {0x00, 0x02}, 0x20004004},
    };
    ...
    for (size_t d = 0; d < BM13XX_HARNESS_DRIVER_COUNT; d++) {
        ...
        for (size_t c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
            ...
            TEST_ASSERT_EQUAL_HEX32(
                cases[c].expected_version,
                asic_rolled_version_from_bits(cases[c].base_version,
...
TEST_CASE("BM13xx register replies do not carry job version fields",
          "[asic][version-rolling][characterization]")
{
    for (size_t d = 0; d < BM13XX_HARNESS_DRIVER_COUNT; d++) {
        const bm13xx_harness_driver_t *driver = &bm13xx_harness_drivers[d];
        ...
        // Job validity is resolved by the result task, so any job frame decodes.
        ...
        TEST_ASSERT_EQUAL_HEX32(0, result->version_bits);
          ...
          "[asic][version-rolling][characterization]")
{
    ...
    for (unsigned failures = 0; failures <= 3; failures++) {
        ...
        TEST_ASSERT_EQUAL_INT(failures < 3 ? ESP_OK : ESP_FAIL,
          ...
          "[asic][version-rolling][stratum][characterization]")
{
    ...
    for (size_t d = 0; d < BM13XX_HARNESS_DRIVER_COUNT; d++) {
        ...
        const task_result *result = driver->process_work(state);
        TEST_ASSERT_NOT_NULL(result);
        const uint32_t rolled_version = asic_rolled_version_from_bits(
            base_version, BIP320_VERSION_ROLLING_MASK, result->version_bits);
        // BIP310 submits the masked field of the rolled version.
        const uint32_t version_bits = rolled_version & BIP320_VERSION_ROLLING_MASK;
        ...
        int sv1_length = STRATUM_V1_submit_share(
            ...
            0x64658bd8, result->nonce, true, version_bits, NULL);
