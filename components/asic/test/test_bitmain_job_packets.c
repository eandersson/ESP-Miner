...
{
    bm_job *job = allocate_bm_job("packet-job", "0001020304050607");
          ...
          "[asic][job-packet][characterization]")
{
    ...
    for (size_t index = 0; index < BM13XX_HARNESS_DRIVER_COUNT; ++index) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(
            driver->send_work(state, job, bm13xx_harness_generation()), driver->name);
        ...
        TEST_ASSERT_TRUE(bm13xx_harness_job_valid(expected[index].job_id));
        bm13xx_harness_end();
    }
}
          ...
          "[asic][job-packet][version-rolling][characterization]")
{
    ...
    bm_job *job = make_job();
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, job, bm1397_harness_generation()));
    ...
    for (uint8_t index = 0; index < 4; ++index) {
        ...
        // The driver reports which midstate matched; the result task rebuilds
        // the version from its own snapshot of the job.
        TEST_ASSERT_EQUAL_UINT8(index, result->version_rolling_index);
        TEST_ASSERT_EQUAL_HEX32(
            expected_versions[index],
            asic_rolled_version_from_midstate(job->version, job->version_mask,
                                              result->version_rolling_index));
        TEST_ASSERT_TRUE(result->timestamp_us == UINT64_C(123456789));
    }
    bm1397_harness_end();
}

TEST_CASE("Bitmain replacements retire the previous owner of a work slot",
          "[asic][job-store][characterization]")
{
    ...
    for (size_t index = 0; index < BM13XX_HARNESS_DRIVER_COUNT; ++index) {
        ...
        TEST_ASSERT_TRUE(driver->send_work(state, first, bm13xx_harness_generation()));
        uint8_t first_id = bm13xx_harness_packet(0)->bytes[4];
        uint8_t replacement_id =
            (uint8_t)((first_id + job_id_steps[index]) % 128);
        bm_job *previous = make_job();
        bm13xx_harness_install_job(replacement_id, previous);

        bm_job *replacement = make_job();
        TEST_ASSERT_TRUE(driver->send_work(state, replacement, bm13xx_harness_generation()));
        ...
        // A nonce still in flight for the old owner must remain resolvable.
        TEST_ASSERT_EQUAL_PTR_MESSAGE(
            previous, bm13xx_harness_retired_job(replacement_id), driver->name);
        bm13xx_harness_end();
    }
    ...
    bm1397_harness_clear_packets();
    bm_job *first = make_job();
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, first, bm1397_harness_generation()));
    uint8_t first_id = bm1397_harness_packet(0)->bytes[4];
    uint8_t replacement_id = (uint8_t)((first_id + 4) % 128);
    bm_job *previous = make_job();
    bm1397_harness_install_job(replacement_id, previous);

    bm_job *replacement = make_job();
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, replacement, bm1397_harness_generation()));
    ...
    TEST_ASSERT_EQUAL_PTR(previous, bm1397_harness_retired_job(replacement_id));
    bm1397_harness_end();
}
          ...
          "[asic][result][register][characterization]")
{
    ...
    TEST_ASSERT_EQUAL_UINT8(0, result->version_rolling_index);
...
TEST_CASE("BM1397 reports job results without consulting the job store",
          "[asic][result][job-store]")
{
    // Slot validity, retired owners and repeated nonces are resolved by the
    // result task against its job snapshot, so the driver decodes every frame.
    GlobalState *state = bm1397_harness_begin();
    TEST_ASSERT_EQUAL_UINT8(2, bm1397_harness_driver.init(state));
    bm1397_harness_clear_packets();

    TEST_ASSERT_NULL(bm1397_harness_driver.process_work(state));
    queue_bm1397_job_response(0x7c, 1, 0x13579bdf);
    const task_result *result = bm1397_harness_driver.process_work(state);
    TEST_ASSERT_NOT_NULL(result);
    TEST_ASSERT_EQUAL_HEX8(0x7c, result->job_id);
    TEST_ASSERT_EQUAL_UINT8(1, result->version_rolling_index);
    TEST_ASSERT_EQUAL_HEX32(0x13579bdf, result->nonce);
    TEST_ASSERT_NULL(bm1397_harness_driver.process_work(state));
    bm1397_harness_end();
}

TEST_CASE("Bitmain work is dropped before TX for a stale epoch or stopping ASIC",
          "[asic][job-store][lifecycle]")
{
    for (size_t index = 0; index < BM13XX_HARNESS_DRIVER_COUNT; ++index) {
        const bm13xx_harness_driver_t *driver = &bm13xx_harness_drivers[index];
        GlobalState *state = bm13xx_harness_begin();

        bm_job *stale = make_job();
        TEST_ASSERT_FALSE_MESSAGE(
            driver->send_work(state, stale, bm13xx_harness_generation() + 1), driver->name);
        TEST_ASSERT_EQUAL_UINT32_MESSAGE(0, bm13xx_harness_packet_count(), driver->name);
        release_bm_job(stale);

        bm13xx_harness_set_running(false);
        bm_job *stopping = make_job();
        TEST_ASSERT_FALSE_MESSAGE(
            driver->send_work(state, stopping, bm13xx_harness_generation()), driver->name);
        TEST_ASSERT_EQUAL_UINT32_MESSAGE(0, bm13xx_harness_packet_count(), driver->name);
        release_bm_job(stopping);

        // A failed UART write leaves the slot invalid and the job unowned.
        bm13xx_harness_set_running(true);
        bm13xx_harness_fail_writes(3);
        bm_job *unsent = make_job();
        TEST_ASSERT_FALSE_MESSAGE(
            driver->send_work(state, unsent, bm13xx_harness_generation()), driver->name);
        uint8_t job_id = bm13xx_harness_packet(0)->bytes[4];
        TEST_ASSERT_NULL_MESSAGE(bm13xx_harness_active_job(job_id), driver->name);
        TEST_ASSERT_FALSE_MESSAGE(bm13xx_harness_job_valid(job_id), driver->name);
        release_bm_job(unsent);
        bm13xx_harness_fail_writes(0);
        bm13xx_harness_end();
    }
}
