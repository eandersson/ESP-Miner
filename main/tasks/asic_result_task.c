                                       ...
                                       const bm_job *job)
{
    if (GLOBAL_STATE->DEVICE_CONFIG.family.asic.id == BM1397) {
        return asic_rolled_version_from_midstate(
            job->version, job->version_mask, result->version_rolling_index);
    }
    return asic_rolled_version_from_bits(job->version, job->version_mask,
                                         result->version_bits);
}
...
void ASIC_result_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        bool is_v1_job = active_job->job_type == JOB_TYPE_V1;
        ...
        // BIP310: the V1 pool rebuilds the version from the masked field.
        uint32_t version_bits =
            is_v1_job
        ...
        if (nonce_diff >= required_share_difficulty || solves_block)
        {
            if (!is_v1_job) {
                ...
                // and closes the connection itself on a failed frame.
                uint64_t sent_time_us = 0;
                int ret = stratum_submit_share(GLOBAL_STATE, active_job,
                                               asic_result->nonce,
                                               asic_result->rolled_version,
                                               &sent_time_us);
                if (ret == STRATUM_V2_SUBMIT_STALE) {
                    ...
                    if (sent_time_us >= asic_result->timestamp_us) {
                        GLOBAL_STATE->SYSTEM_MODULE.process_time =
...
void ASIC_v1_share_submit_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        uint64_t sent_time_us = 0;
        int ret = stratum_v1_submit_share_checked(
            GLOBAL_STATE, share.job, share.generation, share.nonce,
            share.version_bits, share.difficulty, &sent_time_us);
