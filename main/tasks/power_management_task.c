...
static bool voltage_request_valid(GlobalState *GLOBAL_STATE, uint16_t value)
{
    ...
    // TPS546_set_vout() rejects anything below the regulator's VOUT_MIN, which
    // would latch a power fault (#1790).
    int16_t regulator_min_mv = VCORE_get_voltage_min_mv(GLOBAL_STATE);
    if (valid && regulator_min_mv > 0 && value < (uint16_t)regulator_min_mv) {
        valid = false;
    }
    return valid;
}
