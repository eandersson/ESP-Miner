...
uint8_t asic_initialize(GlobalState *GLOBAL_STATE, asic_init_mode_t mode, uint32_t stabilization_delay_ms)
{
    ...
    esp_err_t reset_err = entry_valid ? asic_reset(INIT_RESET_LOW_MS, INIT_RESET_RELEASE_MS) : ESP_ERR_INVALID_STATE;
    ...
    for (uint8_t attempt = 1; attempt <= max_attempts; attempt++) {
        if (attempt > 1) {
            ...
            // Same gate as the first reset: a stop requested while probing
            // must win over another reset pulse.
            pthread_mutex_lock(&GLOBAL_STATE->asic_command_lock);
            bool retry_valid =
                asic_lifecycle_get(GLOBAL_STATE) == ASIC_LIFECYCLE_STARTING &&
                !asic_start_cancelled(GLOBAL_STATE, mode);
            esp_err_t retry_err = retry_valid ? SERIAL_set_baud(UART_FREQ)
                                              : ESP_ERR_INVALID_STATE;
            if (retry_err == ESP_OK) {
                retry_err = asic_reset(RETRY_RESET_LOW_MS, RETRY_RESET_RELEASE_MS);
            }
            pthread_mutex_unlock(&GLOBAL_STATE->asic_command_lock);
            if (retry_err != ESP_OK) {
                ESP_LOGE(TAG, "%s retry reset failed", GLOBAL_STATE->DEVICE_CONFIG.family.asic.name);
                return asic_fail_closed(GLOBAL_STATE,
                                        retry_valid ? "ASIC retry reset failed"
                                                    : "ASIC initialization cancelled");
            }
        }

        if (SERIAL_clear_buffer() != ESP_OK) {
            return asic_fail_closed(GLOBAL_STATE, "ASIC UART flush failed");
        }
